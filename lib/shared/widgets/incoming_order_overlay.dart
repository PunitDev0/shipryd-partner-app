...
///
/// Works uniformly over [Order] — it has no idea (and doesn't need one)
/// whether the offer it's showing is a [ParcelOrder] or [RideOrder]. Once
/// accepted, [OrderNavigation] is the single place that decides which
/// details screen to open.
...
class _IncomingOrderOverlayState extends State<IncomingOrderOverlay> {
  ...
  void _decline({bool timedOut = false}) {
    if (_settled) return;
    _settled = true;
    OrderStore.instance.declineOffer(widget.order.id, timedOut: timedOut);
  }
  ...
  Future<void> _accept() async {
    ...
    try {
      await OrderStore.instance.acceptOffer(order.id);
      // Best-effort — the accept itself already succeeded, so a transient
      // failure refreshing the notification list shouldn't surface as an
      // error here.
      unawaited(AppStore.instance.refreshNotifications().catchError((_) {}));
      if (navigator != null) {
        await OrderNavigation.pushDetailsViaNavigatorState(navigator, order);
      }
    } catch (e) {
      OrderStore.instance.declineOffer(order.id, timedOut: true);
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  formatAmount(order.earning),
                  style: GoogleFonts.inter(fontSize: 30, fontWeight: FontWeight.w800),
                ),
                Text(
                  'Estimated earning · ${order.distanceKm.toStringAsFixed(1)} km · ${order.etaMinutes} min',
                ...
                Wrap(
                  ...
                  runSpacing: 8,
                  children: [
                    _Chip(label: order.itemDescription),
                    ...order.extraDetailChips.map((c) => _Chip(label: c)),
                    _Chip(
                      label: order.paymentMode == 'COD'
                          ? 'COD · ${formatAmount(order.codAmount)}'
                          : 'Prepaid',
                    ),
                  ],
                ),
