...
import 'package:partner/features/parcel/presentation/add_proof_screen.dart';
import 'package:partner/features/parcel/presentation/confirm_receive_screen.dart';
...
class _ParcelDetailsScreenState extends State<ParcelDetailsScreen> {
  ...
  void initState() {
    super.initState();
    ParcelController.markViewed(widget.parcelId);
  }

  @override
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: ParcelController.listenable,
      builder: (context, _) {
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (parcel.status != OrderStatus.received &&
                            parcel.status != OrderStatus.canceled) ...[
                            ...
                            children: [
                              Expanded(
                                child: _buildNavButton(
                                  ...
                                  isActive: parcel.status == OrderStatus.pending,
                                ),
                              ),
                              ...
                              Expanded(
                                child: _buildNavButton(
                                  ...
                                  isActive: parcel.status == OrderStatus.pickedUp,
                                ),
                              ),
                            ],
                          ),
                        ...
                        if (parcel.status != OrderStatus.received && parcel.status != OrderStatus.canceled) ...[
                          const SizedBox(height: 20),
                          Container(
                        ...
                        if (parcel.status == OrderStatus.pending) ...[
                          SizedBox(
                            width: double.infinity,
                            height: 54,
                        ...
                        ] else if (parcel.status == OrderStatus.pickedUp) ...[
                          SizedBox(
                            width: double.infinity,
                            height: 54,
                            child: ElevatedButton(
                        ...
                        if (parcel.status == OrderStatus.pending || parcel.status == OrderStatus.pickedUp) ...[
  ...
  Future<void> _confirmCancel(BuildContext context, String parcelId) async {
    ...
    try {
      await ParcelController.cancel(parcelId);
