...
/// Scanning a QR/barcode on a physical package is a parcel-only concept —
/// rides are never scanned — so this always opens [ParcelDetailsScreen];
/// no ride/parcel branch needed. If the backend ever returned a ride for a
/// scanned code, [ParcelController.scanOrCreate] throws rather than
/// silently handing ride data to this screen.
...
class _ScanParcelScreenState extends State<ScanParcelScreen> {
  ...
  Future<void> _handleCode(String code) async {
    if (_handled) return;
    _handled = true;
    final parcel = await ParcelController.scanOrCreate(code);
    if (!mounted) return;
